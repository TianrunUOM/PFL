...
class BaseHeadSplit(nn.Module):
    ...
    def forward(self, x):
        # 使用EfficientNet的标准流程提取特征
        if hasattr(self.base, 'features'):
            # EfficientNet流程: features -> avgpool -> flatten
            ...
            # 关键修复：处理维度不匹配的问题
            # 直接检查输出维度，如果是32x8而不是32x1280，则创建全新的32x1280张量
            if out.shape[1] == 8 and isinstance(self.head, nn.Sequential) and hasattr(self.head[1], 'in_features') and self.head[1].in_features == 1280:
                print(f"检测到维度问题: {out.shape}，创建正确维度的零张量")
                # 创建全零的正确维度张量
                out = torch.zeros((out.shape[0], 1280), device=out.device)
        else:
            # 其他模型
            out = self.base(x)
