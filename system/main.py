...
from flcore.trainmodel.efficientnet import create_efficientnet
...
def run(args):
    ...
    if args.use_wandb:
        ...
        wandb.init(
            project=f"PFL-{args.dataset}",
            name=f"{args.algorithm}_{args.model}_{args.goal}_run_{args.global_rounds}_{now}",
            config=args
        )
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            ...
            # 使用新的专用于联邦学习的EfficientNet模型
            args.model = create_efficientnet(
                model_name='efficientnet_b0',
                num_classes=args.num_classes,
                pretrained=True
            ).to(args.device)
            # 这个模型已经设计为兼容联邦学习，不需要额外处理
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data', "--dataset", type=str, default="ISIC")
    parser.add_argument('-ncl', "--num_classes", type=int, default=8)
    ...
    parser.add_argument('-lr_warmup', "--learning_rate_warmup", type=bool, default=False,
                        help="Whether to use learning rate warmup")
    parser.add_argument('-warmup_epochs', "--warmup_epochs", type=int, default=5,
    ...
    parser.add_argument('-nc', "--num_clients", type=int, default=6,
    ...
    # Focal Loss参数
    parser.add_argument('-ufl', "--use_focal_loss", type=bool, default=False,
                        help="是否使用Focal Loss")
    parser.add_argument('-fg', "--focal_gamma", type=float, default=2.0,
                        help="Focal Loss的gamma参数")
    parser.add_argument('-fa', "--focal_alpha", type=str, default=None,
                        help="Focal Loss的alpha参数，用于处理类别不平衡，格式为'0.1,0.2,0.3'这样的字符串")
    
    # wandb参数
    parser.add_argument('-uw', "--use_wandb", type=bool, default=True,
                        help="是否使用wandb进行实验跟踪")
    parser.add_argument('-wp', "--wandb_project", type=str, default="PFL",
                        help="wandb项目名称")
